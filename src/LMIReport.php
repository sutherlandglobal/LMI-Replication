...
class LMIReport
{
	...
	public function __construct( $sc, $localDB, $nodeID, $nodeRef,$reportArea)
	{
		...
		$delimFactory = new DelimiterFactory(LMIReport::DELIM_ALPHA_LEN, 1);
		$this->delimiter = $delimFactory->genDiverseDelimiter(LMIReport::DELIM_ALPHA_LEN);
	}
	...
	public function runReport()
	{
		...
		if($startDate != "" && $endDate != "")
		{
			...
			$windowEndDate = date_add($windowEndDate, date_interval_create_from_date_string(LMIReport::REPORT_MAX_WINDOW_RANGE_IN_WEEKS . " weeks"));
			...
			$maxReportEndDate = date_add($reportStartDate, date_interval_create_from_date_string(LMIReport::REPORT_MAX_DATE_RANGE_IN_WEEKS . " weeks"));
			...
			try
			{
				...
				(
					...
					strlen($this->data) < LMIReport::MAX_REPORT_DATA_LEN &&
					(
						strlen($this->data) == 0 ||
						(
							(
								strlen($this->data) > 0 &&
								strlen($this->data) < LMIReport::MIN_REPORT_DATA_LEN
							) ||
			...
			{
				echo "Report Error: ", $e->getMessage(), PHP_EOL;
				$this->data = "";
			}
		}
		else
